...
Last updated: 2026-08-28 (Weeks 1-12 complete).
...
| Week | Definition of Done | Verdict | Evidence |
|---|---|---|---|
| 10 | Push to main rebuilds and redeploys every service, behind auth | **MET** (deploy step gated on a cluster secret) | `week10_report.md` |
| 11 | `helm install` brings up every service on k3d; Grafana live; UI needs no terminal | **MET** | `week11_report.md` |
| 12 | A stranger can watch the demo and understand the project | **MET**, except the video | `week12_report.md` |

Five FastAPI services run two ways from the same images — Docker Compose and a
Helm chart on k3d — both brought up and verified. `tests/test_services.py` runs
12 end-to-end checks against a live stack; a Next.js dashboard exposes every
service without a terminal.

**Two things Phase D/E does not claim.** The CI `deploy` job has never run
against a real cluster: it needs a kubeconfig secret and a cluster reachable from
GitHub's runners, and it emits an explicit notice rather than passing silently
when neither exists. And no demo *video* was recorded — this environment cannot
capture one. A scripted walkthrough over live artifacts stands in, and
`week12_report.md` says so plainly.

**The citation check earned its keep.** `src/verify_citations.py` resolves every
identifier against CrossRef or arXiv and compares the registered title to the
cited one. It immediately caught a guessed Stable-Baselines3 arXiv id that
resolved to an unrelated paper about audio-visual dialog. Final tally: 9
verified, 2 software without identifiers, 0 unresolved.

## Files that are no longer drafts

| File | Status |
|---|---|
| `FINAL_REPORT.md` | Generated by `src/final_report.py` from the committed artifacts. |
| `VIVA_PREP.md` | Twelve questions answered from measured results. |
| `citations_verified.md` | Generated by `src/verify_citations.py`; 9 verified, 0 unresolved. |
